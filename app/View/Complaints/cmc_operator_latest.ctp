<!-- Breadcrumb -->
    <?php
      ...
      $this->Html->addCrumb("Cmc Operator New",'javascript:void(0)');
    ?>
<!-- END Breadcrumb -->
...
<!--<div  class="col-sm-13" style="float:left;border:1px solid #f0f0f0;background:#f1f1f1;margin-bottom: 10px;padding: 10px 0 10px 10px;">-->
	...
	<div class=" text-center">
	   <?php 
	   echo $this->Form->create("Complaint",array('url'=>array('action'=>'cmc_operator_new','novalidate' => true),'id'=>'filter_form',"class"=>"form-inline",)); 
	...
	<table id="index-datatables" class="table table-bordered table-hover">
		<thead>
		    <tr>
                ...
                <th><i class="icon-user"></i>Complaint Id</th>
...
window.onload = function() {
	// var cmc_operator_new_table = $('#index-datatables').dataTable(
 //                {
...
//        });
    ...
    // $.post("complaints_ajax", $("#filter_form").serialize(), function(result) {
    //             // ... Process the result ...
    ...
    //         }, 'json');
        ...
        var oSettings = cmc_operator_new_table.fnSettings();
        cmc_operator_new_table.fnClearTable(this);
            
            $.post("complaints_ajax", $("#filter_form").serialize(), function(json) {


                        for (var i=0; i<json.data.length; i++)
                        {
                           cmc_operator_new_table.oApi._fnAddData(oSettings, json.data[i]);
                        }
                            oSettings.aiDisplay = oSettings.aiDisplayMaster.slice();
                            cmc_operator_new_table.fnDraw();
