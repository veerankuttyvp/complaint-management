...
//              echo $this->Form->input("complaint_priority_id",array("options"=>$complaint_priorities,"empty"=>'Select a Priority','required'=>true,"label"=>array("text"=>"Complaint Priority")));
              echo $this->Form->input("mobile_user_mobile_phone_id",array("options"=>$mobile_user_list,"empty"=>"Select a User","label"=>array("text"=>"Center Name")));
              ...
              echo $this->Form->input("colony_name_id",array("options"=>$colonies,"empty"=>'Select a Colony','required'=>true,'class'=>'chosen',"label"=>array("text"=>"Colony Name")));
               echo '<div class="col-sm-5 control-label"><div style="float:left">Address is same as of consumer</div></div><div class="col-sm-7"><input type="checkbox" id="copy_address" style="margin-top:13px;"></div><div class="clearfix"></div>';
              ...
              echo $this->Form->input("source",array("type"=>"hidden","value"=>"subdivision"));
              echo $this->Form->input("subdivision_id",array("type"=>"hidden","value"=>$subdivison_id));
              echo $this->Form->input("complaint_priority_id",array("type"=>"hidden","value"=>"1"));
              echo $this->Form->input("complaint_status_id",array("type"=>"hidden","value"=>"1"));
        ?>
...
window.onload = function() {
	...
	/* Initialize Form Validation */
     var validator = $('#ComplaintSubdivisioncomplaintForm').validate({
		...
		$('#ComplaintBillNo').focusout(function(){
				...
				$.get('<?php echo $this->Html->url("/complaints/check_consumer_bill",true); ?>/'+$(this).val(), function(data){
					...
					$.get('<?php echo $this->Html->url("/complaints/check_bill",true); ?>/'+$('#ComplaintBillNo').val(), function(data){
						...
						if(result.result == 'success'){
							...
							$( "#address" ).html('House#:'+ values.house+' <br>Street# '+values.street+'<br>Block#'+values.block+'<br>'+values.colony_name );
		 ...
		 $('#copy_address').click(function () {
		 	if($("#copy_address").is(':checked'))
			    $("#ComplaintComplaintAddress").val($("#consumer_address").val());  // checked
			else
